...
class PrivanaAPIClient:
    ...
    def _build_auth_headers(self, shared_secret: bytes, method: str, path: str, body: dict | None = None) -> dict:
        """
        ...
        return {
            ...
            "X-Timestamp": ts,
            "X-Nonce": nonce,
            "X-PQC-Session": session_id if False else "",  # overwritten by caller
        }

    def _headers_for(self, shared_secret: bytes, session_id: str, method: str, path: str, body: dict | None = None) -> dict:
        headers = dict(self.headers)
        auth_headers = self._build_auth_headers(shared_secret, method, path, body)
        auth_headers["X-PQC-Session"] = session_id
